...
- Joins: `INNER` (default) and `LEFT`, derived from Apache Ossie relationships. The join graph must be a tree rooted at fact datasets (star or snowflake). Cycles are a validation error.
