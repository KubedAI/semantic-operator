...
**The shape of a model.** See [`examples/starrocks/retail/model/semanticmodel.yaml`](../examples/starrocks/retail/model/semanticmodel.yaml) for a complete worked example.
...
`list_metrics` and `list_dimensions` return these to the agent, so "what is our revenue" grounds onto `total_sales`. You are not prompt-stuffing the schema. In the future you will be able to import descriptions, glossary terms, and PII tags from OpenMetadata or DataHub. See the [roadmap](ROADMAP.md).
...
Roles and the bigger picture: [OVERVIEW.md](OVERVIEW.md). How compilation and governance work under the hood: [ARCHITECTURE.md](ARCHITECTURE.md).
