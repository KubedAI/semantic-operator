...
func (mc *mysqlConn) auth(authData []byte, plugin string) ([]byte, error) {
	...
	// length-encoded token. The JWT is carried in the DSN password field on a
	// per-request connection. The connection must use TLS, because the token
	// crosses the wire in cleartext.
	case "authentication_openid_connect_client":
		if len(mc.cfg.Passwd) == 0 {
			return nil, fmt.Errorf("JWT token is required for the authentication_openid_connect_client plugin")
		}
